...
import { DEFAULT_CONFIG, STATUS_CONFIG, type SessionStatus } from '../types.js';
...
/**
 * 通过 Unix Domain Socket 发送 HTTP 请求
 * 不受代理环境变量影响
 */
async function fetchViaSocket<T>(urlPath: string, options?: { method?: string; body?: string }): Promise<{ ok: boolean; json: () => Promise<T> }> {
...
/**
 * 检查守护进程是否运行（通过 Unix Domain Socket）
 */
async function isDaemonRunning(): Promise<boolean> {
  ...
  try {
    const response = await fetchViaSocket('/api/health');
    return response.ok;
  } catch {
    return false;
  }
...
async function startCommand(): Promise<void> {
  ...
  if (nowRunning) {
    logSuccess('守护进程已启动');
    logInfo(`Socket: ${SOCKET_PATH}`);
    logInfo(`Dashboard: http://localhost:${DEFAULT_CONFIG.port}/dashboard`);
  } else {
    logError('守护进程启动失败');
    process.exit(1);
  }
...
async function stopCommand(): Promise<void> {
  ...
  try {
    // 通过 socket 文件找关联的进程
    // lsof 可以查找监听 Unix socket 的进程
    const { stdout } = await execAsync(`lsof -tiU "${SOCKET_PATH}" 2>/dev/null`);
...
async function statusCommand(): Promise<void> {
  ...
  try {
    const response = await fetchViaSocket<{ success: boolean; data: { status: string; sessions: number } }>('/api/health');
    const data = await response.json();

    if (data.success) {
      log('守护进程状态: 运行中');
      log(`Socket: ${SOCKET_PATH}`);
      log(`Dashboard: http://localhost:${DEFAULT_CONFIG.port}/dashboard`);
...
async function listCommand(verbose: boolean = false): Promise<void> {
  ...
  try {
    const response = await fetchViaSocket<{ success: boolean; data: Array<{
      pid: number;
      project: string;
      status: string;
      terminal: string;
...
async function showSessionDetails(pid: number): Promise<void> {
  try {
    // 获取提问历史
    const promptsResponse = await fetchViaSocket<{ success: boolean; data: Array<{ prompt: string; timestamp: number }> }>(`/api/sessions/${pid}/prompts`);
    ...
    // 获取工具调用历史
    const toolsResponse = await fetchViaSocket<{ success: boolean; data: Array<{
      tool: string;
      status: string;
      duration?: number;
      startedAt: number;
    }> }>(`/api/sessions/${pid}/tools`);
    ...
    if (toolsData.success && toolsData.data.length > 0) {
      ...
      // 统计
      const statsResponse = await fetchViaSocket<{ success: boolean; data: { prompts: number; toolCalls: number; byTool: Record<string, number> } }>(`/api/sessions/${pid}/stats`);
...
通信方式: ${colors.cyan}Unix Domain Socket${colors.reset} (~/.claude-monitor/monitor.sock)
...
${colors.yellow}Socket:${colors.reset} ${SOCKET_PATH}
...
通信方式:
  Hooks → Daemon: Unix Domain Socket (~/.claude-monitor/monitor.sock)
  Dashboard → Daemon: HTTP/WebSocket (http://localhost:${DEFAULT_CONFIG.port})
